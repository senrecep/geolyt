...
| 2026-07-18 | Deploy hardening wrap-up | Expanded /health with DB+Redis readiness (503 on degraded), added SSL/domain guide, adapted humanizer skill, committed CI workflow + Postgres backup script, pushed | Claude Code | packages/api/src/routes/health.ts, packages/jobs/src/index.ts, docs/ssl-domains.md, .claude/skills/humanizer/, .github/workflows/ci.yml, scripts/backup-postgres.sh |
| 2026-07-18 | Plan-vs-code gap analysis | 4 parallel verifier agents + spot checks; found quota enforcement dead, prompt caching absent, CrawlerBlocked continuation missing, delta template orphaned, API tracing unwired, markdown viewer stub, plus doc staleness | Claude Code | (analysis only) |
| 2026-07-18 | Gap closure sweep | Real quota enforcement (usage.audits), usage per-audit/per-model columns + /usage byModel, cached-token capture + thinking config + model-aware pricing in ai-core, degraded audit continuation on blocked crawlers, delta HTML report endpoint, API OTel wiring, markdown report viewer, pino logger, try/catch → Result.try sweep, PROJECT_PLAN/phase docs realigned | Claude Code | packages/* (see commits), PROJECT_PLAN.md, plans/phase-1.md, plans/phase-2.md |
